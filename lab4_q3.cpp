...
int top, front;
class Issue//class issue with name,phone no and description of issue
{
    public:
    char name[30];
    long int ph;
...
class StkPal//class with pop and push functions
{
    public:
    char NM[30];
    void push(char a)
...
class CustomerService//class with various functions
{
    public:
    Issue* head;
    Issue* tail;
...
int CustomerService::chkPalindrome(char nm[30])//to check whether pallindrome
{
   top=-1;
   front=0;
   char b;
...
void CustomerService::addIssue(char nm[],long int p, char des[])//to add new issue
{
...
void CustomerService::displayIssue()//to display elements
{
    Issue* iss;
    iss=head;
    if(iss==NULL)
...
int main()
{
    CustomerService CS;//element of customer care
    CS.displayIssue();
